...
- **Zero hardcoded values.** Every threshold, model id, effort level, budget, and path lives in `config/classes.yaml` or `config/settings.yaml`. No constant in `router/*.py`.
...
**Interfaces:**
- Consumes: nothing.
- Produces: `ClassSpec` (frozen dataclass: `name: str`, `patterns: tuple[re.Pattern, ...]`, `delegation: str`, `sub_model: str | None`, `sub_effort: str | None`, `budget_soft: int`, `escalation: str`); `Settings` (frozen dataclass: `enforce: bool`, `confidence_threshold: float`, `continuation_max_chars: int`, `continuation_min_turns: int`, `state_dir: Path`, `telemetry_path: Path`, `haiku_model: str`, `haiku_timeout_s: int`, `user_override_patterns: tuple[re.Pattern, ...]`, `agent_override_marker: str`); `load_classes(base: Path, overlay: Path | None) -> dict[str, ClassSpec]`; `load_settings(path: Path) -> Settings`; `load_scope_policy(base: Path) -> str`; `ConfigError(Exception)`.
...
# A prompt shorter than this, in a session past continuation_min_turns, with no
# new URL or issue reference, is treated as a follow-up.
continuation_max_chars: 200
continuation_min_turns: 20
...
def load_settings(path: Path) -> Settings:
    ...
    try:
        patterns = tuple(
            re.compile(p) for p in _require(raw, "user_override_patterns", where)
        )
    except re.error as exc:
        raise ConfigError(f"bad regex in user_override_patterns: {exc}") from exc
    return Settings(
        ...
        continuation_min_turns=int(_require(raw, "continuation_min_turns", where)),
        ...
        user_override_patterns=patterns,
...
**Interfaces:**
- Consumes: `ClassSpec`, `load_classes` from Task 1.
- Produces: `Classification` (frozen dataclass: `cls: str`, `confidence: float`, `source: str`); `classify(prompt: str, classes: dict[str, ClassSpec]) -> Classification`; `UNCLASSIFIED: str = "unclassified"`.

Confidence is deliberately explainable: exactly one class matching scores `0.9`; several matching scores `0.6` and the class with the most matching patterns wins (ties broken by config order); nothing matching scores `0.0` and yields `UNCLASSIFIED`. With `confidence_threshold: 0.7`, only a clean single match skips refinement.
...
def test_single_match_is_confident():
    r = classify("Resolve issue https://example.com/org/repo/issues/1088", CLASSES)
...
def test_ambiguous_prompt_scores_below_threshold():
    ...
    r = classify("Implement the release automation for the deploy", CLASSES)
    assert r.confidence == 0.6, "multi-class match must invite refinement"
    assert r.cls in {"mechanical", "feature"}


def test_review_of_a_pull_request_is_pr_review():
    r = classify("Please review https://example.com/org/repo/pull/1056", CLASSES)
...
def test_golden_set_coverage_does_not_regress():
    ...
    hits = sum(1 for r in rows if classify(r["prompt"], CLASSES).cls == r["class"])
    coverage = hits / len(rows)
    print(f"\ngolden fixture: {path.name}  coverage={coverage:.1%} ({hits}/{len(rows)})")
    assert coverage >= 0.85, f"coverage regressed to {coverage:.1%} using {path.name}"
...
def classify(prompt: str, classes: dict[str, ClassSpec]) -> Classification:
    ...
    if not matched:
        return Classification(UNCLASSIFIED, CONFIDENCE_NONE, "none")
    ...
    confidence = CONFIDENCE_SINGLE if len(matched) == 1 else CONFIDENCE_MULTI
...
Expected: PASS, 6 passed. Output includes `golden fixture: golden.example.jsonl coverage=100.0%`
...
def test_short_followup_in_a_long_session_is_continuation():
    st = _state()
    st.transcript_offset = 999_999
...
_NEW_WORK_MARKERS = re.compile(r"https?://|\bissues?/\d+|\bpull/\d+|#\d{2,}")
...
def is_continuation(
    ...
    if len(prompt) > settings.continuation_max_chars:
        return False
    if _NEW_WORK_MARKERS.search(prompt):
        return False
    return state.transcript_offset > 0
```
...
- Produces: `router/paths.py` exposing `REPO: Path`, `CLASSES_YAML: Path`, `CLASSES_LOCAL_YAML: Path`, `SETTINGS_YAML: Path`, `state_dir_override() -> Path | None` (test-only env override, extended with `enforce_override()` in Task 8 and `telemetry_override()` in Task 9); an executable hook reading hook JSON on stdin and writing `{"hookSpecificOutput": {"hookEventName": "UserPromptSubmit", "additionalContext": ...}}` on stdout.
...
def test_followup_reuses_the_stored_class(tmp_path):
    ...
    state["transcript_offset"] = 500_000
...
- [ ] **Step 3: Write `router/paths.py`**

```python
# router/paths.py
"""Canonical config locations, and the test-only state-dir override."""
...
def main() -> None:
    ...
    if is_continuation(prompt, prior, settings) and prior is not None:
        result = Classification(prior.cls, prior.confidence, "continuation")
        state = prior
    else:
        result = classify(prompt, classes)
    ...
    print(json.dumps({
        "hookSpecificOutput": {
            ...
            "additionalContext": render(result, spec, policy),
        }
    }))
...
```bash
git add router/paths.py hooks/user_prompt_submit.py tests/test_hook_prompt_submit.py
...
any error: exit 0, no stdout, session unaffected.
"""
...
CHIP_REASON = (
    "Router: unrelated findings do not become background-task chips. "
    "File it with `gh issue create` and continue the current task."
)
...
def main() -> None:
    ...
    if tool_name == CHIP_TOOL and enforce:
        out["permissionDecision"] = "deny"
        out["permissionDecisionReason"] = CHIP_REASON
    ...
    if tool_name in DISPATCH_TOOLS:
        classes = load_classes(paths.CLASSES_YAML, paths.CLASSES_LOCAL_YAML)
        decision = decide(
            payload.get("tool_input") or {}, classes.get(state.cls), state, settings
        )
        if decision.updated_input is not None:
            state.overrides.append({
                "from": (payload.get("tool_input") or {}).get("model"),
            ...
            if enforce:
                ...
                notes.append(f"ROUTER: {decision.reason}")

    tokens, new_offset = read_increment(Path(payload["transcript_path"]), state.transcript_offset)
    ...
    if (
        ...
        notes.append(
            f"BUDGET: {state.out_tokens} of {state.budget_soft} output tokens for "
            f"class {state.cls}. Stop and present what you have and what you "
            f"propose next, rather than continuing."
        )
...
- Produces: `summarise(rows: list[dict]) -> dict[str, dict]` mapping class name to `{"n", "median", "p75", "exceeded", "overrides"}`; `propose(summary: dict, classes: dict[str, ClassSpec]) -> list[str]`; a `__main__` entry point.
...
from router import paths, telemetry
from router.config import ClassSpec, load_classes, load_settings
...
def propose(summary: dict[str, dict], classes: dict[str, ClassSpec]) -> list[str]:
    lines: list[str] = []
    for cls, stats in sorted(summary.items()):
        spec = classes.get(cls)
        if spec is None or stats["n"] < MIN_SAMPLES_TO_PROPOSE:
            continue
        if stats["exceeded"] / stats["n"] >= 0.5:
            lines.append(
        ...
        if stats["overrides"] / stats["n"] >= 0.5:
            lines.append(
...
def main() -> None:
    ...
    suggestions = propose(summary, load_classes(paths.CLASSES_YAML, paths.CLASSES_LOCAL_YAML))
    if suggestions:
        print("\nSuggestions (apply by hand):")
        for line in suggestions:
            print(f"  - {line}")
    else:
        print(f"\nNo suggestions: fewer than {MIN_SAMPLES_TO_PROPOSE} sessions per class.")
...
- Produces: `build_prompt(prompt: str, class_names: list[str]) -> str`; `parse_response(text: str, class_names: list[str]) -> str | None`; `refine(prompt: str, class_names: list[str], settings: Settings, runner=...) -> str | None`. `runner` is an injected callable `(list[str], int) -> str` so tests never spawn the CLI.
...
FAKE_ENV = "TASK_ROUTER_FAKE_HAIKU"
...
def main() -> int:
    ...
    spec = classes[cls]
    state.cls = cls
    state.confidence = REFINED_CONFIDENCE
    state.source = "haiku"
    state.budget_soft = spec.budget_soft
    save_state(state_dir, state)

    contract = render(
        Classification(cls, REFINED_CONFIDENCE, "haiku"),
        spec,
...
- [ ] **Step 6: Verify the real CLI path once, by hand**
...
```bash
git add router/llm.py hooks/refine_class.py tests/test_llm.py
